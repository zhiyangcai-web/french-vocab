# Daily Active Vocabulary Log

用途：记录每天真正需要变成主动词汇的表达。  
原则：只收“可直接用于写作/口语”的表达，不收孤立单词。

## 2026-04-27 - Services publics et dématérialisation

### Active Expressions

1. `faire des démarches administratives`
   - 批次：2026-04-27
   - 标签：administration, services-publics, demarches
   - 中文：办理行政手续
   - 搭配：`faire ses démarches administratives en ligne`
   - 例句：`J'en ai assez de faire des démarches administratives en ligne.`

2. `la prise de rendez-vous en ligne`
   - 批次：2026-04-27
   - 标签：rendez-vous, services-publics, demarches-en-ligne
   - 中文：线上预约
   - 搭配：`la prise de rendez-vous en ligne est difficile`
   - 例句：`La prise de rendez-vous en ligne est devenue un véritable casse-tête.`

3. `une plateforme constamment saturée`
   - 批次：2026-04-27
   - 标签：plateforme-numerique, services-publics, dysfonctionnement
   - 中文：总是爆满/无法正常使用的平台
   - 搭配：`dénoncer une plateforme constamment saturée`
   - 例句：`L'ANEF est une plateforme constamment saturée.`

4. `un véritable casse-tête`
   - 批次：2026-04-27
   - 标签：difficulte, probleme-pratique, argumentation
   - 中文：非常头疼的问题
   - 搭配：`devenir un véritable casse-tête`
   - 例句：`Pour certains usagers, les démarches en ligne deviennent un véritable casse-tête.`

5. `une rupture de soins`
   - 批次：2026-04-27
   - 标签：sante, acces-aux-soins, vulnerabilite
   - 中文：医疗/治疗中断
   - 搭配：`provoquer une rupture de soins`
   - 例句：`Une rupture de soins peut avoir des conséquences graves.`

6. `être placé malgré soi en situation irrégulière`
   - 批次：2026-04-27
   - 标签：statut-administratif, titre-de-sejour, inegalites
   - 中文：非自愿地陷入无合法身份状态
   - 搭配：`être placé malgré soi en situation irrégulière à cause de...`
   - 例句：`Des personnes sont placées malgré elles en situation irrégulière à cause des blocages administratifs.`

7. `se retrouver malgré soi en situation irrégulière`
   - 批次：2026-04-27
   - 标签：statut-administratif, titre-de-sejour, temoignage
   - 中文：非自愿地变成身份不规则/无合法身份
   - 搭配：`je me retrouve malgré moi en situation irrégulière`
   - 例句：`Je me retrouve malgré moi en situation irrégulière, alors que j'attends une prolongation de mon titre de séjour.`

8. `renouveler un titre de séjour`
   - 批次：2026-04-27
   - 标签：titre-de-sejour, administration, demarches
   - 中文：更新/续办居留证
   - 搭配：`renouveler son titre de séjour`
   - 例句：`Il est parfois difficile de renouveler son titre de séjour sur une plateforme saturée.`

9. `être à l'aise avec le numérique`
   - 批次：2026-04-27
   - 标签：numerique, inclusion-numerique, competences
   - 中文：熟悉/擅长使用数字工具
   - 搭配：`ne pas être à l'aise avec le numérique`
   - 例句：`Les personnes âgées ne sont pas toujours à l'aise avec le numérique.`

10. `être exclu des services publics`
   - 批次：2026-04-27
   - 标签：services-publics, exclusion, inegalites
   - 中文：被公共服务排除在外
   - 搭配：`risquer d'être exclu des services publics`
   - 例句：`Certaines personnes risquent d'être exclues des services publics.`

## 2026-05-19 - Éducation et apprentissage numérique

### Active Expressions

1. `suivre un cours en ligne`
   - 批次：2026-05-19
   - 标签：education, apprentissage-en-ligne, numerique
   - 中文：上网课
   - 搭配：`suivre une partie de ses cours en ligne`
   - 例句：`De plus en plus d'étudiants suivent une partie de leurs cours en ligne.`

2. `acquérir de nouvelles compétences`
   - 批次：2026-05-19
   - 标签：competences, formation, developpement-personnel
   - 中文：获得新技能
   - 搭配：`acquérir des compétences utiles`
   - 例句：`Les cours en ligne permettent d'acquérir de nouvelles compétences à son rythme.`

3. `favoriser l'autonomie des étudiants`
   - 批次：2026-05-19
   - 标签：autonomie, education, apprentissage
   - 中文：促进学生自主性
   - 搭配：`favoriser l'autonomie des étudiants`
   - 例句：`L'apprentissage en ligne peut favoriser l'autonomie des étudiants.`

4. `creuser les inégalités scolaires`
   - 批次：2026-05-19
   - 标签：inegalites-scolaires, education, fracture-numerique
   - 中文：加深教育不平等
   - 搭配：`risquer de creuser les inégalités scolaires`
   - 例句：`Le numérique risque de creuser les inégalités scolaires si tous les élèves n'ont pas accès aux mêmes outils.`

5. `manquer d'encadrement`
   - 批次：2026-05-19
   - 标签：encadrement, education, apprentissage-a-distance
   - 中文：缺乏指导/监督
   - 搭配：`certains étudiants manquent d'encadrement`
   - 例句：`Certains étudiants manquent d'encadrement lorsqu'ils suivent des cours à distance.`

6. `rester motivé sur le long terme`
   - 批次：2026-05-19
   - 标签：motivation, education, apprentissage
   - 中文：长期保持动力
   - 搭配：`avoir du mal à rester motivé sur le long terme`
   - 例句：`Il peut être difficile de rester motivé sur le long terme sans contact direct avec les enseignants.`

7. `avoir accès à des ressources pédagogiques`
   - 批次：2026-05-19
   - 标签：ressources-pedagogiques, education, acces
   - 中文：能够获得教学资源
   - 搭配：`avoir accès à des ressources pédagogiques de qualité`
   - 例句：`Internet permet d'avoir accès à des ressources pédagogiques très variées.`

8. `adapter son rythme d'apprentissage à ses besoins`
   - 批次：2026-05-19
   - 标签：rythme-apprentissage, education, individualisation
   - 中文：根据自己的需要调整学习节奏
   - 搭配：`adapter son rythme d'apprentissage à ses besoins`
   - 例句：`Les plateformes en ligne permettent parfois d'adapter son rythme d'apprentissage à ses besoins.`

9. `mettre en place un accompagnement personnalisé`
   - 批次：2026-05-19
   - 标签：accompagnement-personnalise, education, prevention-du-decrochage
   - 中文：建立个性化辅导/支持
   - 搭配：`mettre en place un accompagnement personnalisé pour les étudiants`
   - 例句：`Les universités devraient mettre en place un accompagnement personnalisé pour éviter le décrochage.`

10. `évaluer les progrès des apprenants`
   - 批次：2026-05-19
   - 标签：evaluation, education, progres
   - 中文：评估学习者的进步
   - 搭配：`évaluer régulièrement les progrès des apprenants`
   - 例句：`Il est important d'évaluer régulièrement les progrès des apprenants.`

## 2026-05-19 - Travail et organisation professionnelle

### Active Expressions

1. `concilier vie professionnelle et vie personnelle`
   - 批次：2026-05-19
   - 标签：equilibre-vie-privee, travail, teletravail
   - 中文：平衡工作与个人生活
   - 搭配：`concilier vie professionnelle et vie personnelle`
   - 例句：`Le télétravail peut aider certains salariés à concilier vie professionnelle et vie personnelle.`

2. `améliorer les conditions de travail`
   - 批次：2026-05-19
   - 标签：conditions-de-travail, travail, bien-etre
   - 中文：改善工作条件
   - 搭配：`améliorer les conditions de travail des salariés`
   - 例句：`Les entreprises devraient améliorer les conditions de travail des salariés.`

3. `être confronté à une forte pression`
   - 批次：2026-05-19
   - 标签：pression-professionnelle, travail, sante-mentale
   - 中文：面临很大压力
   - 搭配：`être confronté à une forte pression au travail`
   - 例句：`De nombreux employés sont confrontés à une forte pression au travail.`

4. `prévenir l'épuisement professionnel`
   - 批次：2026-05-19
   - 标签：sante-au-travail, epuisement-professionnel, prevention
   - 中文：预防职业倦怠
   - 搭配：`prévenir l'épuisement professionnel des salariés`
   - 例句：`Il est nécessaire de prévenir l'épuisement professionnel.`

5. `renforcer la cohésion d'équipe`
   - 批次：2026-05-19
   - 标签：cohesion-equipe, travail, management
   - 中文：加强团队凝聚力
   - 搭配：`renforcer la cohésion d'équipe`
   - 例句：`Les réunions en présentiel peuvent renforcer la cohésion d'équipe.`

6. `gagner en efficacité`
   - 批次：2026-05-19
   - 标签：efficacite, organisation, travail
   - 中文：提高效率
   - 搭配：`gagner en efficacité grâce à une meilleure organisation`
   - 例句：`Une organisation plus claire permet de gagner en efficacité.`

7. `bénéficier d'horaires flexibles`
   - 批次：2026-05-19
   - 标签：horaires-flexibles, travail, autonomie
   - 中文：享有灵活工作时间
   - 搭配：`bénéficier d'horaires flexibles`
   - 例句：`Les salariés souhaitent souvent bénéficier d'horaires flexibles.`

8. `remettre en question l'organisation du travail`
   - 批次：2026-05-19
   - 标签：organisation-du-travail, reforme, travail
   - 中文：重新审视工作组织方式
   - 搭配：`remettre en question l'organisation du travail`
   - 例句：`La semaine de quatre jours remet en question l'organisation du travail.`

9. `réduire le temps de trajet`
   - 批次：2026-05-19
   - 标签：temps-de-trajet, teletravail, qualite-de-vie
   - 中文：减少通勤时间
   - 搭配：`réduire le temps de trajet quotidien`
   - 例句：`Le télétravail permet de réduire le temps de trajet quotidien.`

10. `mettre en place le droit à la déconnexion`
   - 批次：2026-05-19
   - 标签：droit-a-la-deconnexion, droits-des-salaries, numerique
   - 中文：建立离线权/下班不被打扰权
   - 搭配：`mettre en place le droit à la déconnexion`
   - 例句：`Il faudrait mettre en place le droit à la déconnexion pour protéger les salariés.`

## 2026-05-20 - Consommation et pouvoir d'achat

### Active Expressions

1. `faire face à une hausse du coût de la vie`
   - 批次：2026-05-20
   - 标签：cout-de-la-vie, pouvoir-dachat, inflation
   - 中文：应对生活成本上涨
   - 搭配：`faire face à une forte hausse du coût de la vie`
   - 例句：`De nombreux ménages ont du mal à faire face à une hausse du coût de la vie.`

## 2026-05-20 - Culture et vie sociale

### Active Expressions

1. `renforcer la cohésion sociale`
   - 批次：2026-05-20
   - 标签：cohesion-sociale, solidarite, vivre-ensemble
   - 中文：加强社会凝聚力
   - 搭配：`renforcer la cohésion sociale dans les quartiers`
   - 例句：`Les associations locales peuvent renforcer la cohésion sociale en créant des espaces de rencontre.`

2. `préserver le lien social`
   - 批次：2026-05-20
   - 标签：lien-social, isolement, solidarite
   - 中文：维护社会联系/人际纽带
   - 搭配：`préserver le lien social entre les générations`
   - 例句：`Les activités collectives permettent de préserver le lien social et de lutter contre l'isolement.`

## 2026-05-20 - Environnement et transition écologique

### Active Expressions

1. `réduire les émissions de gaz à effet de serre`
   - 批次：2026-05-20
   - 标签：gaz-a-effet-de-serre, climat, transition-ecologique
   - 中文：减少温室气体排放
   - 搭配：`réduire les émissions de gaz à effet de serre dans les transports`
   - 例句：`Pour lutter contre le réchauffement climatique, il faut réduire les émissions de gaz à effet de serre.`

2. `adopter des gestes écoresponsables`
   - 批次：2026-05-20
   - 标签：gestes-ecoresponsables, consommation-responsable, environnement
   - 中文：采取环保负责任的日常行为
   - 搭配：`adopter des gestes écoresponsables au quotidien`
   - 例句：`Chacun peut adopter des gestes écoresponsables, comme réduire sa consommation d'énergie.`

## 2026-05-20 - Santé et accès aux soins

### Active Expressions

1. `avoir du mal à accéder aux soins`
   - 批次：2026-05-20
   - 标签：acces-aux-soins, inegalites-territoriales, sante
   - 中文：难以获得医疗服务
   - 搭配：`avoir du mal à accéder aux soins dans les zones rurales`
   - 例句：`Dans certaines régions, de nombreux patients ont du mal à accéder aux soins faute de médecins.`

## 2026-05-20 - Médias et information

### Active Expressions

1. `lutter contre la désinformation`
   - 批次：2026-05-20
   - 标签：desinformation, medias, esprit-critique
   - 中文：打击虚假信息
   - 搭配：`lutter contre la désinformation sur les réseaux sociaux`
   - 例句：`L'école devrait apprendre aux jeunes à lutter contre la désinformation en développant leur esprit critique.`

## 2026-05-20 - Travail et organisation professionnelle

### Active Expressions

1. `favoriser l'insertion professionnelle des jeunes`
   - 批次：2026-05-20
   - 标签：insertion-professionnelle, jeunes, emploi
   - 中文：促进年轻人就业融入
   - 搭配：`favoriser l'insertion professionnelle des jeunes diplômés`
   - 例句：`Les stages et l'alternance peuvent favoriser l'insertion professionnelle des jeunes.`

## 2026-05-20 - Logement et conditions de vie

### Active Expressions

1. `souffrir d'un manque de logements abordables`
   - 批次：2026-05-20
   - 标签：logement-abordable, conditions-de-vie, precarite
   - 中文：面临可负担住房不足的问题
   - 搭配：`souffrir d'un manque de logements abordables dans les grandes villes`
   - 例句：`Les grandes villes souffrent souvent d'un manque de logements abordables pour les étudiants et les jeunes actifs.`

## 2026-05-20 - Numérique et société

### Active Expressions

1. `encadrer l'usage des écrans chez les adolescents`
   - 批次：2026-05-20
   - 标签：ecrans, adolescents, usage-du-numerique
   - 中文：规范青少年的屏幕使用
   - 搭配：`encadrer l'usage des écrans chez les adolescents`
   - 例句：`Il est nécessaire d'encadrer l'usage des écrans chez les adolescents sans diaboliser le numérique.`

## 2026-05-27 - Énergie et maintenance industrielle

### Active Expressions

1. `assurer la maintenance préventive d'une éolienne`
   - 批次：Stage CGN - maintenance énergie
   - 标签：eolien, maintenance, prevention
   - 中文：对风机进行预防性维护
   - 搭配：`assurer la maintenance préventive d'une éolienne`
   - 例句：`L'équipe assure la maintenance préventive des éoliennes afin d'éviter les arrêts imprévus.`

2. `effectuer une maintenance corrective après une panne`
   - 批次：Stage CGN - maintenance énergie
   - 标签：depannage, panne, maintenance
   - 中文：故障后进行纠正性维护/维修
   - 搭配：`effectuer une maintenance corrective après une panne`
   - 例句：`Après l'alarme, les techniciens ont effectué une maintenance corrective sur le système de contrôle.`

3. `diagnostiquer l'origine d'un défaut`
   - 批次：Stage CGN - maintenance énergie
   - 标签：diagnostic, defaut, terrain
   - 中文：诊断故障/缺陷的原因
   - 搭配：`diagnostiquer l'origine d'un défaut`
   - 例句：`Avant de remplacer la pièce, il faut diagnostiquer l'origine du défaut avec les données disponibles.`

4. `mettre une installation hors tension`
   - 批次：Stage CGN - maintenance énergie
   - 标签：securite, electricite, intervention
   - 中文：让设备/装置断电
   - 搭配：`mettre une installation hors tension avant intervention`
   - 例句：`Avant toute intervention électrique, l'installation doit être mise hors tension.`

5. `consigner un équipement avant intervention`
   - 批次：Stage CGN - maintenance énergie
   - 标签：consignation, securite, electricite
   - 中文：检修前对设备进行挂牌上锁/能量隔离
   - 搭配：`consigner un équipement avant intervention`
   - 例句：`Le technicien doit consigner l'équipement avant intervention pour éviter toute remise sous tension.`

6. `vérifier l'absence de tension`
   - 批次：Stage CGN - maintenance énergie
   - 标签：vat, securite, electricite
   - 中文：验电/确认无电压
   - 搭配：`vérifier l'absence de tension avec un appareil adapté`
   - 例句：`Même après la coupure, il faut vérifier l'absence de tension avant de toucher l'équipement.`

7. `porter les équipements de protection individuelle`
   - 批次：Stage CGN - maintenance énergie
   - 标签：epi, securite, terrain
   - 中文：佩戴个人防护装备
   - 搭配：`porter les équipements de protection individuelle`
   - 例句：`Sur le site, le port des équipements de protection individuelle est obligatoire.`

8. `contrôler l'état des pales et de la nacelle`
   - 批次：Stage CGN - maintenance énergie
   - 标签：eolien, inspection, composants
   - 中文：检查叶片和机舱状态
   - 搭配：`contrôler l'état des pales`
   - 例句：`Pendant l'inspection, l'équipe contrôle l'état des pales et de la nacelle.`

9. `surveiller les paramètres de fonctionnement`
   - 批次：Stage CGN - maintenance énergie
   - 标签：parametres, supervision, exploitation
   - 中文：监测运行参数
   - 搭配：`surveiller les paramètres de fonctionnement en temps réel`
   - 例句：`La salle de contrôle surveille les paramètres de fonctionnement pour détecter les anomalies.`

10. `réagir à une alarme de défaut`
   - 批次：Stage CGN - maintenance énergie
   - 标签：alarme, exploitation, reaction
   - 中文：响应故障报警
   - 搭配：`réagir rapidement à une alarme de défaut`
   - 例句：`Lorsqu'une alarme de défaut apparaît, il faut vérifier les données avant d'envoyer une équipe sur site.`

11. `remplacer une pièce défectueuse`
   - 批次：Stage CGN - maintenance énergie
   - 标签：piece, reparation, logistique
   - 中文：更换有故障的零件
   - 搭配：`remplacer une pièce défectueuse`
   - 例句：`Si le capteur est confirmé défaillant, il faudra remplacer la pièce défectueuse.`

12. `planifier un arrêt de production`
   - 批次：Stage CGN - maintenance énergie
   - 标签：production, arret, coordination
   - 中文：安排停产/停机检修
   - 搭配：`planifier un arrêt de production`
   - 例句：`Nous devons planifier un arrêt de production pour réaliser l'intervention en sécurité.`

13. `raccorder un parc éolien au réseau électrique`
   - 批次：Stage CGN - maintenance énergie
   - 标签：reseau, eolien, electricite
   - 中文：将风电场接入电网
   - 搭配：`raccorder un parc éolien au réseau électrique`
   - 例句：`L'électricité produite par le parc éolien est raccordée au réseau électrique via un transformateur.`

14. `faire un retour d'intervention`
   - 批次：Stage CGN - maintenance énergie
   - 标签：rapport, communication, terrain
   - 中文：做检修/现场作业反馈
   - 搭配：`faire un retour d'intervention à l'équipe`
   - 例句：`Après la visite, le technicien fait un retour d'intervention au responsable de maintenance.`

15. `remettre l'installation en service`
   - 批次：Stage CGN - maintenance énergie
   - 标签：centrale, exploitation, securite
   - 中文：恢复设备/装置运行
   - 搭配：`remettre l'installation en service après contrôle`
   - 例句：`Après les essais, l'équipe peut remettre l'installation en service si tous les paramètres sont conformes.`

## 2026-05-27 - Budget et contrôle de gestion industriel

### Active Expressions

1. `élaborer un budget prévisionnel`
   - 批次：Stage CGN - budget énergie
   - 标签：budget, prevision, controle-de-gestion
   - 中文：编制预算预测/预算草案
   - 搭配：`élaborer un budget prévisionnel pour l'année prochaine`
   - 例句：`Le service budget doit élaborer un budget prévisionnel en tenant compte des besoins de maintenance.`

2. `suivre l'exécution budgétaire`
   - 批次：Stage CGN - budget énergie
   - 标签：budget, execution, suivi
   - 中文：跟踪预算执行情况
   - 搭配：`suivre l'exécution budgétaire chaque mois`
   - 例句：`Chaque mois, nous suivons l'exécution budgétaire des projets de maintenance.`

3. `analyser les écarts entre le budget et le réalisé`
   - 批次：Stage CGN - budget énergie
   - 标签：ecarts, analyse, reporting
   - 中文：分析预算与实际之间的差异
   - 搭配：`analyser les écarts entre le budget et le réalisé`
   - 例句：`Il faut analyser les écarts entre le budget et le réalisé avant la réunion mensuelle.`

4. `distinguer les dépenses d'exploitation et les dépenses d'investissement`
   - 批次：Stage CGN - budget énergie
   - 标签：opex, capex, classification
   - 中文：区分运营支出和投资支出
   - 搭配：`distinguer les dépenses d'exploitation et les dépenses d'investissement`
   - 例句：`Avant de valider la demande, il faut distinguer les dépenses d'exploitation et les dépenses d'investissement.`

5. `imputer une dépense à un centre de coût`
   - 批次：Stage CGN - budget énergie
   - 标签：centre-de-cout, imputation, couts
   - 中文：把费用归集到成本中心
   - 搭配：`imputer une dépense à un centre de coût`
   - 例句：`Cette facture doit être imputée au centre de coût de la maintenance éolienne.`

6. `préparer un reporting budgétaire mensuel`
   - 批次：Stage CGN - budget énergie
   - 标签：reporting, mensuel, pilotage
   - 中文：准备月度预算报告
   - 搭配：`préparer un reporting budgétaire mensuel`
   - 例句：`Je prépare un reporting budgétaire mensuel pour suivre les dépenses par projet.`

7. `maîtriser les coûts de maintenance`
   - 批次：Stage CGN - budget énergie
   - 标签：couts, maintenance, optimisation
   - 中文：控制维护成本
   - 搭配：`maîtriser les coûts de maintenance`
   - 例句：`L'objectif est de maîtriser les coûts de maintenance sans compromettre la sécurité des installations.`

8. `valider une demande d'achat`
   - 批次：Stage CGN - budget énergie
   - 标签：achats, validation, procedure
   - 中文：审批采购申请
   - 搭配：`valider une demande d'achat dans le système`
   - 例句：`Avant de valider une demande d'achat, le service budget vérifie que les crédits sont disponibles.`

9. `suivre les dépenses liées aux pièces de rechange`
   - 批次：Stage CGN - budget énergie
   - 标签：pieces-de-rechange, stocks, maintenance
   - 中文：跟踪备件相关支出
   - 搭配：`suivre les dépenses liées aux pièces de rechange`
   - 例句：`Nous devons suivre les dépenses liées aux pièces de rechange, car elles pèsent sur le budget de maintenance.`

10. `justifier un dépassement budgétaire`
   - 批次：Stage CGN - budget énergie
   - 标签：depassement, justification, budget
   - 中文：解释/证明预算超支原因
   - 搭配：`justifier un dépassement budgétaire`
   - 例句：`Le responsable doit justifier le dépassement budgétaire lié à une réparation urgente.`

11. `respecter l'enveloppe budgétaire`
   - 批次：Stage CGN - budget énergie
   - 标签：enveloppe, arbitrage, priorite
   - 中文：遵守预算额度
   - 搭配：`respecter l'enveloppe budgétaire allouée`
   - 例句：`Pour ce projet, il faut respecter l'enveloppe budgétaire allouée par la direction.`

12. `réviser les prévisions budgétaires`
   - 批次：Stage CGN - budget énergie
   - 标签：forecast, prevision, ajustement
   - 中文：修订预算预测
   - 搭配：`réviser les prévisions budgétaires en cours d'année`
   - 例句：`Après la hausse du coût des pièces, nous devons réviser les prévisions budgétaires.`

13. `consolider les données financières`
   - 批次：Stage CGN - budget énergie
   - 标签：donnees, consolidation, excel
   - 中文：汇总/合并财务数据
   - 搭配：`consolider les données financières de plusieurs sites`
   - 例句：`Avant le reporting, il faut consolider les données financières transmises par les sites.`

14. `évaluer le coût d'un arrêt de production`
   - 批次：Stage CGN - budget énergie
   - 标签：arret-production, cout, analyse
   - 中文：评估停机/停产成本
   - 搭配：`évaluer le coût d'un arrêt de production`
   - 例句：`Pour décider du calendrier de maintenance, il faut évaluer le coût d'un arrêt de production.`

15. `engager une dépense`
   - 批次：Stage CGN - budget énergie
   - 标签：engagement, depense, procedure
   - 中文：发生/承诺一笔支出
   - 搭配：`engager une dépense dans la limite du budget`
   - 例句：`Une dépense ne peut être engagée que si elle respecte la procédure et l'enveloppe disponible.`

